...
resource "aws_instance" "ec2-appa" {
  ami           = local.ec2-appa.ami
  instance_type = local.ec2-appa.inst_type
  subnet_id     = module.vpc.private_subnets[0]
  vpc_security_group_ids = [
  ...
  root_block_device {
    delete_on_termination = local.ec2-appa.root_device.delete_on_termination
    encrypted             = local.ec2-appa.root_device.encrypted
    iops                  = local.ec2-appa.root_device.iops
    throughput            = local.ec2-appa.root_device.throughput
    volume_size           = local.ec2-appa.root_device.volume_size
    volume_type           = local.ec2-appa.root_device.volume_type
    tags                  = merge({ Name = local.ec2-appa.name, DeployedBy = local.ec2-appa.DeployedBy }, local.tags)
  }
  tags = merge({ Name = local.ec2-appa.name, DeployedBy = local.ec2-appa.DeployedBy }, local.tags)
}
